    ...
    @if(isset($root))
        <div class="panel panel-warning">
            <div class="panel-heading">
                <span class="badge pull-left">@if($root->category) {{ $root->category->name }} @endif</span>
                <h3 class="panel-title">{{ $root->title }}</h3>
            </div>
            <div class="panel-body">{{ $root->content }}</div>
            <div class="panel-footer text-left">
                <small>{{ trans('irticket::ticket.created_by') }} {{ $root->user->name }} {{ trans('irticket::ticket.on') }} {{ $root->created_at }}</small>
            </div>
        </div>
        @if($children)
